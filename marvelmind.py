...
class MarvelmindHedge (Thread):
    def __init__ (self, adr=2, tty="/dev/ttyACM2", baud=9600, maxvaluescount=3, debug=False, recieveUltrasoundPositionCallback=None, recieveImuRawDataCallback=None, recieveImuDataCallback=None, recieveUltrasoundRawDataCallback=None):
        self.tty = tty  # serial
    ...
    def returnx(self):
        ...
        self.x = np.array([self.position()[1]])
        print("X = ", self.x)
        #else:
         #   self.x = np.array([self.position()[1]])
          #  print("X = ", self.x)
            
        #self.x = self.x * 3.28084
        #print(self.x)
     
    def returny(self):
        self.y = np.array([self.position()[2]])
        print("Y = ", self.y)
        #self.y = self.y * 3.28084
        #print(self.y)
